...
def test_create_manifest(tmp_path, monkeypatch) -> None:
    ...
    manifest = download_module.create_manifest(
    ...
    expected_manifest = {
        "dataset_metadata": {"dataset_name": "test-owner/test-dataset", "dataset_version": 1},
        "orders": {
...
def test_create_manifest_raise_error_if_file_size_is_wrong(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="file size does not match"):
        download_module.create_manifest(
            dataset_metadata=dataset_metadata,
            dataset_version=1,
            download_date=download_date,
            settings=Settings(
...
def test_save_manifest_creates_directory_and_file(tmp_path) -> None:
    ...
    download_module.save_manifest(
        manifest_name="benchmark_raw_data_manifest.json",
        manifest=manifest,
