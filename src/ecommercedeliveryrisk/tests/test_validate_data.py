...
def valid_case(tmp_path, monkeypatch):
    ...
    benchmark = {
        "dataset_metadata": {"dataset_name": settings.kaggle_dataset, "dataset_version": 1},
        "orders": {
...
def test_compare_manifests_with_valid_data(valid_case):
    settings, benchmark, _ = valid_case
...
def test_validate_data_with_wrong_file_metric(valid_case, key, wrong_value, error, message):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "wrong_dataset",
            ValueError,
            "The manifest data does not match the benchmark data.\n",
        ),
        (
            "dataset_metadata",
            "dataset_version",
        ...
        (
            "orders",
            "column_names",
            ["wrong_id", "wrong_status"],
            ValueError,
            "The manifest data does not match the benchmark data.\n",
        ),
    ],
)
def test_compare_manifests_with_wrong_file_metric(
    valid_case, file_id, key, wrong_value, error, message
):
    settings, benchmark, _ = valid_case
