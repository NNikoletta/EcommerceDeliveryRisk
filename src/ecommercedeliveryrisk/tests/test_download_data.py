...
def test_download_raw_data_directory_is_empty_no_benchmark(tmp_path, monkeypatch) -> None:
    ...
    settings = Settings(kaggle_dataset='test-owner/test-dataset',
                        ...
                        manifests_data_dir=test_manifest_dir)
    ...
    download_module.download_raw_data(settings=settings)

    expected_dataset = f"{settings.kaggle_dataset}/1"
    ...
    mock_api.dataset_status.assert_called_once_with(dataset=settings.kaggle_dataset, format="json(current_version_number)")

    mock_api.dataset_download_files.assert_called_once_with(dataset=expected_dataset, path=settings.raw_data_dir, unzip=True)
...
def test_download_raw_data_directory_is_empty_benchmark_exists(tmp_path, monkeypatch) -> None:
    ...
    mock_benchmark_path = settings.manifests_data_dir / "benchmark_raw_data_manifest.json"
...
def test_download_raw_data_directory_is_not_empty_replacement_disabled(tmp_path, monkeypatch) -> None:
    ...
    mock_file_path = settings.raw_data_dir / "mock_file.csv"
    ...
    #  Calling the download with the temporary test directory for the raw data
    download_module.download_raw_data(settings=settings, replace_existing=False)
