...
def test_download_raw_data(
    ...
    outcome = download_module.download_raw_data(
        settings=settings, replace_existing=replace_existing
    )
    ...
    if contains_old_data and benchmark_available:
        ...
        download.assert_called_once_with(settings=settings, data_dir=staging_dir, dataset_version=1)
...
def test_replacement_failure_preserves_original_and_cleans_staging(
    settings, pipeline_mocks, failure_stage
):
    download, create, validate = pipeline_mocks
    settings.raw_data_dir.mkdir()
    settings.manifests_data_dir.mkdir()
