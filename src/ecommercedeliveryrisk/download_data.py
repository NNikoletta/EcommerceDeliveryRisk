...
from ecommercedeliveryrisk.config import (
    DownloadResult,
    ExpectedFiles,
    FileManifest,
    Settings,
...
def download_raw_data(
    ...
    benchmark_manifest_name: str = "benchmark_raw_data_manifest.json",
) -> DownloadOutcome:
    ...
    if not any(settings.raw_data_dir.iterdir()):
        benchmark = load_manifest(
            manifests_dir=settings.manifests_data_dir, manifest_name=benchmark_manifest_name
        )
        if benchmark is not None:
            download_results = download_kaggle_dataset(
                settings=settings, dataset_version=benchmark["dataset_metadata"]["dataset_version"]
            )
        else:
        ...
        download_results = replace_raw_data(
            settings=settings, benchmark_manifest_name=benchmark_manifest_name
        )
    ...
    manifest = create_manifest(
        dataset_metadata=download_results.dataset_metadata,
    ...
    save_manifest(
        ...
        manifest=manifest,
        input_manifest_data_dir=settings.manifests_data_dir,
    )
...
def download_kaggle_dataset(
    settings: Settings, data_dir=None, dataset_version=None
) -> DownloadResult:
    ...
    if dataset_version is None:
        dataset_version = json.loads(
            api.dataset_status(
                dataset=settings.kaggle_dataset, format="json(current_version_number)"
            )
        )
    ...
    return DownloadResult(
        download_date=time, dataset_metadata=all_csv_metadata, dataset_version=dataset_version
    )
...
def replace_raw_data(
    settings: Settings, benchmark_manifest_name: str
):  # only replaces main Settings data
    benchmark = load_manifest(
        manifests_dir=settings.manifests_data_dir, manifest_name=benchmark_manifest_name
    )
    if benchmark is not None:
        ...
        with tempfile.TemporaryDirectory(dir=settings.raw_data_dir.parent) as tmp_path:
            tmp_raw_data_dir = Path(tmp_path)
            download_results = download_kaggle_dataset(settings=settings, data_dir=tmp_raw_data_dir)
...
def create_manifest(
    dataset_metadata: list[dict], dataset_version: int, download_date: str, settings: Settings
) -> dict:
    expected_files = asdict(ExpectedFiles())
    manifest: dict[
        str,
        dict[str, str | int] | FileManifest,
    ] = {
        "dataset_metadata": {
            "dataset_name": settings.kaggle_dataset,
...
def save_manifest(manifest_name: str, manifest: dict, input_manifest_data_dir) -> None:
    ...
    if file_path.is_file():
        logger.info(
            f"Manifest under the name: '{manifest_name}' already exists; new manifest will be saved as 'tmp_raw_data_manifest.json'."
        )
        manifest_name = "tmp_raw_data_manifest.json"
        file_path = input_manifest_data_dir / manifest_name
        if file_path.is_file():
            logger.info(
                "Temporary manifest already exists and will be overwritten with the newly created manifest."
            )
            file_path.unlink()
